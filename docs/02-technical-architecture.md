...
## 0. ما نُفّذ فعليًا (web/)

التنفيذ الحالي يتبع هذه الوثيقة مع ثلاثة فروق مقصودة لتبسيط التشغيل الأول:

| في الوثيقة | في التنفيذ | السبب |
|---|---|---|
| Monorepo بحزم منفصلة | تطبيق Next.js 15 واحد في `web/`، والمنطق النقي في `src/core` | أسرع للتشغيل والصيانة في هذه المرحلة؛ الفصل ممكن لاحقًا بلا تغيير في المنطق |
| PostgreSQL | SQLite محليًا عبر Prisma، والتحويل إلى PostgreSQL بتغيير `provider` | تشغيل بلا أي خدمة خارجية |
| Redis + BullMQ | ناقل أحداث ومجدول داخل عملية الخادم، والمهام محفوظة في جدول `EscalationJob` | يكفي نسخة واحدة؛ المهام لا تضيع عند إعادة التشغيل |
| مصادقة OTP بالبريد | بريد وكلمة مرور (scrypt) | لا يحتاج مزوّد بريد |

كل مفتاح خارجي (SMS، واتساب) يُدخل من لوحة المدير ويُخزَّن مشفّرًا (AES-256-GCM) في جدول `IntegrationSetting`. مفاتيح إشعارات الويب (VAPID) تُولَّد تلقائيًا.
